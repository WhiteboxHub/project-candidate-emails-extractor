...
class FilterRepository:
    ...
    def _is_dynamic_junk(self, local_part: str, domain: str) -> bool:
        ...
        # 1. High digit density (5+ digits in localpart)
        ...
        # 2. Random hex/hash strings (8+ chars of 0-9a-f)
        # Often used by tracking/reply systems
        if re.search(r'[a-f0-9]{12,}', local_part):
            return True
            
        # 3. Known bot prefixes followed by numbers
        # v-123..., tr-123..., id-123...
        if re.match(r'^(?:v|tr|ref|id|cid|bounce|u|s|ext)-[0-9]+', local_part):
            return True
            
        # 4. Excessive subdomains (bot-like domain nesting)
        if domain.count('.') >= 4:
            return True
            
        # 5. Generic bot pattern: letters + numbers at the end
